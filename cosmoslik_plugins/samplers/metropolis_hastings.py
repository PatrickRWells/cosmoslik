...
from cosmoslik.chains import Chain, Chains, combine_covs, get_covariance, load_chain
...
class metropolis_hastings(SlikSampler):
    ...
    def restart_sample(self, lnl):
        chain = load_chain(self.output_file)
        print("Chain loaded")
        sample = chain[0].get_last()
        self.x0 = [sample[k] for k in self.sampled]
        return self._mpi_mcmc(self.x0,lnl, restart=True)
    ...
    def _mpi_mcmc(self,x,lnl,restart=False):
        ...
        if self.output_file is not None:
            if not restart:
                ...
                try:
                    self._output_file = open(self.output_file,"ab")
                except:
                    print(("Error: unable to open chain file %s")%(self.output_file))

                cp_split = os.path.basename(self.output_file).split('.')
                cp_split[0] += "_copy"
                cp_out = '/'.join([os.path.dirname(self.output_file),'.'.join(cp_split)] )
                print("Restarting chain. New sample will be appended to the existing chain file. This feature has not yet been rigourously tested.")
                print(("To avoid data loss in event of a crash, the original chain has been copied to %s")%(cp_out))
                cmd = ' '.join(['cp', self.output_file, cp_out])
                os.system(cmd)
                ...
                while True:
                    ...
                    for i, s in zip(list(range(self.mpi_comm_freq)),sampler):
                        if self._is_restarting:
                            ...
                            continue
                        yield s
                        samples[i-1 if restart and first else i] = tuple(chain(*[[s.lnl,s.weight],s.x,[s.extra[k] for k in self.output_extra_params]]))
