...
class Chain(dict):
    ...
    def get_last(self):
        """return the last sample in the chain. Used when restarting a chain"""
        length = self.length()
        return_dict = {}
        for key in self.keys():
            return_dict.update({key: self.get(key)[length-1]})
        return return_dict
...
class Chains(list):
    ...
    def saveall(self, file):
        """output all chains to files, formatted for GetDist"""
        for c, i in enumerate(self):
            ofile = os.path.split(file)
            print(ofile[1])
